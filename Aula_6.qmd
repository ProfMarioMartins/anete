...
Parafraseie o parágrafo abaixo. Siga as estratégias anteriormente apresentadas. Trata-se do primeiro parágrafo do artigo científico "A formação de professores e o uso do ChatGPT® para revisão de textos", escrito por Larissa Alvarenga de Souza Honorato, Helena Maria Ferreira, Jaciluz Dias e publicado em 2024. A reposta deve ser inserida no [Painel de Respostas](portifolio.qmd). Selecione a tarefa "Paráfrase".
...
Essa prática envolve tanto a **citação direta** (ou discurso direto), quando se reproduz fielmente as palavras de outrem, quanto a **citação indireta** (ou \*discurso indireto), quando se parafraseiam as palavras de outrem com as próprias palavras.
...
::: citacao
(...) uma referência é um “Conjunto padronizado de elementos descritivos, retirados de um documento, que per-mite sua identificação individual”, ou seja, são todas as informações sobre a obra que está sendo utilizada no trabalho. (Castro _et al_, 2019, p. 80 - aspas dos autores).
:::
...
-   Escolha um dos artigos selecionados na aula [7 ("Como selecionar fontes confiáveis?)](Aula_5.qmd) e leia-o atentamente.
:::
